...
class SRFBN(basic_network):
    ...
    def train_step(self):
        ...
        l2_regularization_loss = tf.reduce_sum(tf.get_collection("weights_l2_loss"))

        self.losses = [self.calc_loss(x=x, y=self.labelplaceholder, loss_type=self.cfg.loss_type) for x in self.outs]
        self.losses = tf.reduce_sum(self.losses)/len(self.losses)/self.cfg.batchsize + l2_regularization_loss
